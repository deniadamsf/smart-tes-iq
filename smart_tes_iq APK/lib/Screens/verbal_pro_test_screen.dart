...
import '../data/verbal_pro_data.dart';
import '../helpers/database_helper.dart';
import '../helpers/test_label_helper.dart'; // BARU: Untuk terjemahan judul tes

class VerbalProTestScreen extends StatefulWidget {
  const VerbalProTestScreen({super.key});

  @override
  State<VerbalProTestScreen> createState() => _VerbalProTestScreenState();
}

class _VerbalProTestScreenState extends State<VerbalProTestScreen> {
  bool _isTestStarted = false;

  int _currentIndex = 0;
  final Map<int, String> _answers = {};

  int _timeLeft = 600; // 10 Menit
  Timer? _timer;
  ...
  void initState() {
    ...
    List<Map<String, dynamic>> allQuestions = List.from(VerbalProData.questions);
    ...
    if (_currentIndex < _questions.length - 1) {
      ...
      if (_answers.length < _questions.length) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('verbal_pro.msg_incomplete'.tr()), backgroundColor: Colors.red)
        );
        return;
      }
      _finishTest();
  ...
  Future<void> _finishTest() async {
    ...
    for (int i = 0; i < _questions.length; i++) {
      if (_answers.containsKey(i)) {
        // Amankan pencocokan dengan mengambil awalan jawaban (contoh: "A.", "B.")
    ...
    // SIMPAN KE DATABASE LOKAL DENGAN NAMA STATIS INDONESIA
    await DatabaseHelper.instance.insertTestResult({
      'user_id': currentUserId,
      'test_name': 'Analogi Verbal (PRO)',
      'score': score,
    ...
    if (!completed.contains('Analogi Verbal (PRO)')) {
      completed.add('Analogi Verbal (PRO)');
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('verbal_pro.msg_finished'.tr()), backgroundColor: Colors.green)
    );
      ...
      child: Padding(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.assignment_late_outlined, size: 50, color: Color(0xFF0D47A1)),
              const SizedBox(height: 10),
              Text('verbal_pro.instruction_title'.tr(), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF333333))),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    _buildRuleRow(Icons.format_list_numbered, 'verbal_pro.rule_questions'.tr(args: [_questions.length.toString()])),
                    const SizedBox(height: 10),
                    _buildRuleRow(Icons.timer, 'verbal_pro.rule_time'.tr()),
                    const SizedBox(height: 10),
                    _buildRuleRow(Icons.touch_app, 'verbal_pro.rule_next'.tr()),
                  ],
                ),
              ),
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text('verbal_pro.tips_title'.tr(), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFFE65100))),
                      ],
                    ),
                    const SizedBox(height: 10),
                    Text(
                      'verbal_pro.tips_content'.tr(),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text('verbal_pro.btn_start'.tr(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                ),
              )
            ],
          ),
  ...
  Widget build(BuildContext context) {
    if (_questions.isEmpty) return Scaffold(body: Center(child: Text('verbal_pro.msg_empty_bank'.tr())));
    ...
    final displayTestName = getLocalizedTestLabel('Analogi Verbal (PRO)', langCode);
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  'verbal_pro.test_instruction'.tr(),
                ...
                child: Container(
                  ...
                  child: Center(
                    child: SingleChildScrollView(
                      child: Text(displayQuestion, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, height: 1.4), textAlign: TextAlign.center),
                    ),
                  ),
                ),
              ),
              ...
              Expanded(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // Validasi cek substring(0,2) misal "A." untuk keamanan bahasa
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text('verbal_pro.btn_prev'.tr(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ),
                  ),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text(_currentIndex < _questions.length - 1 ? 'verbal_pro.btn_next'.tr() : 'verbal_pro.btn_finish'.tr(), style: const TextStyle(fontSize: 16, color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                  ),
                ],
              ),
