...
class SpasialTestScreen extends StatefulWidget {
  const SpasialTestScreen({super.key});

  @override
  State<SpasialTestScreen> createState() => _SpasialTestScreenState();
}

class _SpasialTestScreenState extends State<SpasialTestScreen> {
  int _currentIndex = 0;
  ...
  void initState() {
    ...
    _questions = List<Map<String, dynamic>>.from(SpasialData.questions);
    _questions.shuffle();
    ...
    if (_currentIndex < _questions.length - 1) {
      ...
      if (_answers.length < _questions.length) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text('spasial.msg_incomplete'.tr()),
  ...
  Future<void> _finishTest({required bool timeUp}) async {
    ...
    // PENTING: String 'Spasial (Gambar)' tetap statis untuk Database
    await DatabaseHelper.instance.insertTestResult({
      'test_name': 'Spasial (Gambar)',
      'score': finalScore,
    ...
    Navigator.pushReplacement(
      ...
      MaterialPageRoute(
        builder: (context) => ResultScreen(
          testName: 'Spasial (Gambar)', // Tetap dikirim versi statisnya
          score: finalScore,
  ...
  Widget build(BuildContext context) {
    ...
    final displayTestName = getLocalizedTestLabel('Spasial (Gambar)', context.locale.languageCode);

    if (_questions.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: Text(displayTestName), backgroundColor: const Color(0xFF0D47A1)),
        body: Center(child: Text('spasial.msg_empty_bank'.tr())),
      );
    }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // BARISAN TOMBOL PILIHAN JAWABAN (A, B, C)
              Row(
              ...
              // ROW TOMBOL SEBELUMNYA & SELANJUTNYA
              Row(
                children: [
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text('spasial.btn_prev'.tr(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ),
                  ),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text(
                        _currentIndex < _questions.length - 1 ? 'spasial.btn_next'.tr() : 'spasial.btn_finish'.tr(),
