...
import '../data/penalaran_logis_data.dart';
...
class PenalaranLogisTestScreen extends StatefulWidget {
  const PenalaranLogisTestScreen({super.key});

  @override
  State<PenalaranLogisTestScreen> createState() => _PenalaranLogisTestScreenState();
}

class _PenalaranLogisTestScreenState extends State<PenalaranLogisTestScreen> {
  int _currentIndex = 0;
  ...
  void initState() {
    ...
    // 1. Ambil semua soal dari data
    List<Map<String, dynamic>> allQuestions = List.from(PenalaranLogisData.questions);

    // 2. Acak seluruh soal
    allQuestions.shuffle();

    // 3. Ambil 25 soal saja
    _questions = allQuestions.take(25).toList();
    ...
    if (_currentIndex < _questions.length - 1) {
      ...
      if (_answers.length < _questions.length) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text('logika.msg_incomplete'.tr()),
  ...
  int _calculateFinalScore() {
    ...
    for (int i = 0; i < _questions.length; i++) {
      if (_answers.containsKey(i)) {
        // Amankan pengecekan jawaban dwibahasa dengan mengambil 1 huruf pertama saja
  ...
  Future<void> _finishTest({required bool timeUp}) async {
    ...
    // PENTING: String 'Penalaran Logis' tetap statis agar database tidak corrupt
    await DatabaseHelper.instance.insertTestResult({
      'test_name': 'Penalaran Logis',
      'score': finalScore,
    ...
    Navigator.pushReplacement(
      ...
      MaterialPageRoute(
        builder: (context) => ResultScreen(
          testName: 'Penalaran Logis', // Tetap dilempar dalam bahasa Indonesia
          score: finalScore,
  ...
  Widget build(BuildContext context) {
    ...
    final displayTestName = getLocalizedTestLabel('Penalaran Logis', langCode);

    if (_questions.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: Text(displayTestName), backgroundColor: const Color(0xFF0D47A1)),
        body: Center(child: Text('logika.msg_empty_bank'.tr())),
      );
    }

    final currentQ = _questions[_currentIndex];

    // Logika fallback: Siap membaca data lama maupun data dwibahasa baru
              ...
              Expanded(
                ...
                child: Container(
                  ...
                  child: SingleChildScrollView(
                    child: Text(
                      displayQuestion,
                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600, height: 1.5, color: Color(0xFF333333)),
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text('logika.btn_prev'.tr(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ),
                  ),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text(
                        _currentIndex < _questions.length - 1 ? 'logika.btn_next'.tr() : 'logika.btn_finish'.tr(),
